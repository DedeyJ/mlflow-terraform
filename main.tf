...
resource "docker_image" "script_image" {
  ...
  build {
    context    = abspath("${path.module}/Churning")  # Path to the directory containing the Dockerfile
    dockerfile = "Dockerfile"  # Name of the Dockerfile
  }
}
...
resource "docker_container" "script_container" {
  ...
  volumes {
    host_path       = abspath("${path.module}/Churning/src")
    container_path  = "/src"
  }

  # Define the second volume
  volumes {
    host_path       = abspath("${path.module}/Churning/data")
    container_path  = "/data"
  }
}