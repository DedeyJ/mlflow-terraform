...
def objective(trial):
    ...
    return 1 - accuracy  # Optuna minimizes the objective function, so we return 1 - accuracy, this can be changed depending on which parameter you evaluate the model.
...
with mlflow.start_run(experiment_id=experiment_id, run_name=run_name, nested=True) as run:
    ...
    #log the best model in the model registry.
